...
int nextIndex( int index ) {
	return ((++index) % QUEUE_SIZE); // modulo gurantees circularity of list(if next index > size, new index jumps back to first)
}
...
int Queue_IsEmpty( const Queue *queue ) {
	return (queue->firstIndex == queue->freeIndex) ? 1 : 0; //if firstIndex == freeIndex <=> list is empty
}
...
int Queue_IsFull( const Queue *queue ) {
	return (nextIndex(queue->freeIndex)) == (queue->firstIndex) ? 1 : 0; //if last array index is used, nextindex should move index back to first index -> list is full
}
...
void Queue_Remove( Queue *queue ) {
	...
	queue->firstIndex = nextIndex(queue->firstIndex); //moves first index to next position
}
